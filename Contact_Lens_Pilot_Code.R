...
GC_compounds <- mz1_gc1$Metabolite_name

gc_inchikey_list<- list()

# Loop through each metabolite name

pb <- progress_bar$new(total = length(GC_compounds)) #progress bar for loop processing

for (i in seq_along(GC_compounds)) {
  pb$tick()
  tryCatch({ #can do without the tryCatch but it's just better at handling errors#
    props <- get_properties(
      properties = c("InChI", 'SMILES'),
      identifier = c(GC_compounds[i]),
      namespace = "name")
    if (!is.null(props)) {
      gc_inchikey <- retrieve(props, .combine.all = TRUE, .to.data.frame = TRUE)
      gc_inchikey_list[[i]] <- gc_inchikey
    } else {
      message(paste("No result found for:", GC_compounds[i]))
    }
  }, error = function(e) {
    message(paste("Error processing:", GC_compounds[i], "Error:", e$message))
  })
}

GC_IDs_Combined  <- bind_rows(gc_inchikey_list) %>%
...
nrow(mz1_gc_inchikey_merge)-nrow(mz1_gc1) #sanity check should give 0

mz1_gc_missed_inchikey<-mz1_gc_inchikey_merge %>%
...
Missing_GC_inchikeys <- read_csv("/Users/anyaguo/Library/CloudStorage/OneDrive-McMasterUniversity/Results/Missing INCHKEY and SMILES (2025 02 26)(Sheet1)(in).csv", col_types = cols(.default = "c"))
Missing_GC_inchikeys_subset <- Missing_GC_inchikeys %>%
  subset(select = c(Metabolite_name, SMILES, INCHIKEY)) %>%
  group_by(Metabolite_name) %>% slice(1)

mz1_gc_missed_inchikey2_merge <- merge(mz1_gc_missed_inchikey2, Missing_GC_inchikeys_subset, by = c('Metabolite_name'), all = T) %>%
  filter(!is.na(Average_Mz))

mz1_gc <- rbind(mz1_gc_with_inchikey, mz1_gc_missed_inchikey2_merge) %>%
  subset(select = -c(CID, IsomericSMILES, InChI))
#write.csv(mz1_gc,"/Users/anyaguo/Library/CloudStorage/OneDrive-McMasterUniversity/Results/mz1_gc.csv", row.names=FALSE) #export final data frame#
